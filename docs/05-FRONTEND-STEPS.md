...
- [ ] Authorize channels with a custom Echo `authorizer` that POSTs to `/api/v1/broadcasting/auth` **through the app's own axios client**, not with `authEndpoint`. Two things are wrong with the endpoint route: Echo's default (`/broadcasting/auth` on the `web` group) cannot see a Sanctum SPA session, and `authEndpoint` — even repointed at `/api/v1` — issues the request from pusher-js's own XHR, which sends the session cookie but **not** the `X-XSRF-TOKEN` header. Laravel's CSRF middleware then answers every private-channel subscription with a 419 and real-time degrades to silence with nothing in the UI to show it. Routing through the axios client reuses the interceptor that already primes the CSRF cookie, so the two cannot drift. (Corrected 2026-08-21 after the 419 was observed in the browser; `03-FRONTEND-ARCHITECTURE.md` §4.2 carries the same fix.)
